"""
Supabase Budget Operations
Budget management and merchant rules using Supabase as backend.
"""
...
def get_budgets_supabase() -> Dict[str, float]:
...
def save_budget_supabase(category: str, amount: float) -> bool:
...
@st.cache_data(ttl=CACHE_TTL, show_spinner=False)
def load_merchant_rules_supabase() -> Dict[str, str]:
...
def save_merchant_rule_supabase(merchant: str, category: str) -> bool:
    ...
    try:
        ...
        # Clear cache
        load_merchant_rules_supabase.clear()
        return True
    except Exception as e:
